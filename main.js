...
async function testSupabaseSessio() {
  ...
  cardReader.style.background="red";
    } else if (session) {
      ...
      afegirDebugVisual("✅ Sessió activa: " + session.user.email);
        ...
        const cardReader = document.getElementById("card-reader");
  cardReader.style.background="#383535";
    } else {
...
import {logoBase64} from "./config.js";

  

const supabase = createClient(SUPABASE_URL, SUPABASE_KEY);
//if (supabase) {supabase.from("fitxatges").select().limit(1)
 //   .then(({ data, error }) => {
//      if (error) {
...
async function comprovarSessioInicial() {
  ...
  if (!session || error) {
    afegirLiniaTauler("🔐 No hi ha sessió activa");
   mostrarPantallaLogin();
  } else {
    afegirLiniaTauler("✅ Sessió detectada");
    iniciarSessio();
...
function simularLecturaTarjeta(treballador, tipus) {
  ...
  const cardReader = document.getElementById("card-reader");
  cardReader.style.background="green";
...
function iniciarContadorSessio() {
  actualitzarTextBoto();
 const panel=document.getElementById("card-reader");
  intervalSessio = setInterval(() => {
    segonsRestants--;

if(segonsRestants===0){
  afegirLiniaTauler("Sessió caducada: Cal tornar enviar les credencials");

 panel.style.background=" #ad0909ff";
}
    if (segonsRestants <= 0) {
      document.getElementById("loginScreen").style.visibility="visible";
